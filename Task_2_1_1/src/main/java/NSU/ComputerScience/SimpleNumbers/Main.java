...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var seq = benchmarkRunnable(() -> {
            Sequential.SequentialSearch(arr);
        }, 1000);
        var parallelStream = benchmarkRunnable(() -> {
            ParallelStream.ParallelSearch(arr);
        }, 1000);
        var threads_2 = benchmarkRunnable(() -> {
            ParallelThread.ThreadSearch(arr, 2);
        }, 1000);
